...
***********************************************/
int benvel(float *rv1, float *rvo, long n)
{
	int i;
	...
	vec_float v1, v2, v3, v4, v5, v6, v7, v8;
	
	vec_float l1, l2, l3, l4, l5, l6, l7, l8;
	
    vec_float	sq1,  sq2,  sq3,  sq4,  sq5,  sq6,  sq7,  sq8;
	
	 vec_float	sq_1,  sq_2,  sq_3,  sq_4,  sq_5,  sq_6,  sq_7,  sq-8;
	
	vec_float add1, add2, add3, add4, add5, add6, add7, add8;
	
	vec_float sqrt1, sqrt2, sqrt3, sqrt4, sqrt5, sqrt6, sqrt7, sqrt8;
	
	vec_float res1, res2, res3, res4, res5, res6, res7, res8;
    ...
    int prefetch_const = 0x10010100;
     ...
     vec_dst(rv1, prefetch_const, 0);

	 
		 for (i = 0; i < (n/4); i+=8)
		  {

			vec_dst(&rv1[i*4], prefetch_const, 0);

		v1.AsVector = vec_ld(0, &rv1[i*4]);                         /* loading of the input values */
		v2.AsVector = vec_ld(0, &rv1[(i+1)*4]);
		v3.AsVector = vec_ld(0, &rv1[(i+2)*4]);
		v4.AsVector = vec_ld(0, &rv1[(i+3)*4]);
		v5.AsVector = vec_ld(0, &rv1[(i+4)*4]);
		v6.AsVector = vec_ld(0, &rv1[(i+5)*4]);
		v7.AsVector = vec_ld(0, &rv1[(i+6)*4]);
		v8.AsVector = vec_ld(0, &rv1[(i+7)*4]);

		
		l1.AsVector = vec_ld(0, &hlrvo[i*4]);                /* loading the values obtained after hilbert transform of the input values */
		l2.AsVector = vec_ld(0, &hlrvo[(i+1)*4]);
		l3.AsVector = vec_ld(0, &hlrvo[(i+2)*4]);
		l4.AsVector = vec_ld(0, &hlrvo[(i+3)*4]); 
		l5.AsVector = vec_ld(0, &hlrvo[(i+4)*4]);
		l6.AsVector = vec_ld(0, &hlrvo[(i+5)*4]);
		l7.AsVector = vec_ld(0, &hlrvo[(i+6)*4]);
		l8.AsVector = vec_ld(0, &hlrvo[(i+7)*4]);
		
				
        sq1.AsVector = vec_madd(v1.AsVector, v1.Asector, kMinusZero);          /*  squaring of the input values */
        sq2.AsVector = vec_madd(v2.AsVector, v2.Asector, kMinusZero);
        sq3.AsVector = vec_madd(v3.AsVector, v3.Asector, kMinusZero);
        sq4.AsVector = vec_madd(v4.AsVector, v4.Asector, kMinusZero);
        sq5.AsVector = vec_madd(v5.AsVector, v5.Asector, kMinusZero);
        sq6.AsVector = vec_madd(v6.AsVector, v6.Asector, kMinusZero);
        sq7.AsVector = vec_madd(v7.AsVector, v7.Asector, kMinusZero);
        sq8.AsVector = vec_madd(v8.AsVector, v8.Asector, kMinusZero);

		
        sq_1.AsVector = vec_madd(l1.AsVector, l1.Asector, kMinusZero);         /*  squaring of the values obtained after hilbert transform of the input values */
        sq_2.AsVector = vec_madd(l2.AsVector, l2.Asector, kMinusZero);
        sq_3.AsVector = vec_madd(l3.AsVector, l3.Asector, kMinusZero);
        sq_4.AsVector = vec_madd(l4.AsVector, l4.Asector, kMinusZero);
        sq_5.AsVector = vec_madd(l5.AsVector, l5.Asector, kMinusZero);
        sq_6.AsVector = vec_madd(l6.AsVector, l6.Asector, kMinusZero);
        sq_7.AsVector = vec_madd(l7.AsVector, l7.Asector, kMinusZero);
        sq_8.AsVector = vec_madd(l8.AsVector, l8.Asector, kMinusZero);
			
			
		add1.AsVector = vec_add(sq1.AsVector, sq_1.AsVector);           	/* addition of the squared terms  */
		add2.AsVector = vec_add(sq2.AsVector, sq_2.AsVector);
        add3.AsVector = vec_add(sq3.AsVector, sq_3.AsVector);
        add4.AsVector = vec_add(sq4.AsVector, sq_4.AsVector);
        add5.AsVector = vec_add(sq5.AsVector, sq_5.AsVector);
        add6.AsVector = vec_add(sq6.AsVector, sq_6.AsVector);
		add7.AsVector = vec_add(sq7.AsVector, sq_7.AsVector);
        add8.AsVector = vec_add(sq8.AsVector, sq_8.AsVector);
		
		
		sqrt1.AsVector = vec_rsqrte(add1.AsVector);
		sqrt2.AsVector = vec_rsqrte(add2.AsVector);
		sqrt3.AsVector = vec_rsqrte(add3.AsVector);
		sqrt4.AsVector = vec_rsqrte(add4.AsVector);
		sqrt5.AsVector = vec_rsqrte(add5.AsVector);
		sqrt6.AsVector = vec_rsqrte(add6.AsVector);
		sqrt7.AsVector = vec_rsqrte(add7.AsVector);
		sqrt8.AsVector = vec_rsqrte(add8.AsVector);
		
		
		res1.AsVector = vec_re(sqrt1.AsVector);
		res2.AsVector = vec_re(sqrt2.AsVector);
		res3.AsVector = vec_re(sqrt3.AsVector);
		res4.AsVector = vec_re(sqrt4.AsVector);
		res5.AsVector = vec_re(sqrt5.AsVector);
		res6.AsVector = vec_re(sqrt6.AsVector);
		res7.AsVector = vec_re(sqrt7.AsVector);
		res8.AsVector = vec_re(sqrt8.AsVector);
		
		
		vec_st(res1.AsVector, 0, &rvo[i*4]);
		vec_st(res2.AsVector, 0, &rvo[(i+1)*4]);
		vec_st(res3.AsVector, 0, &rvo[(i+2)*4]);
		vec_st(res4.AsVector, 0, &rvo[(i+3)*4]);
		vec_st(res5.AsVector, 0, &rvo[(i+4)*4]);
		vec_st(res6.AsVector, 0, &rvo[(i+5)*4]);
		vec_st(res7.AsVector, 0, &rvo[(i+6)*4]);
		vec_st(res8.AsVector, 0, &rvo[(i+7)*4])
...
int bhlbrt(float *rv1, float *rvo, long n)
{
	...
	vec_float v1, v2, v3, v4, v5, v6, v7, v8;
    vec_float	mul1  mul2  mul3  mul4 mul5  mul6  mul7  mul8;
	...
	fminusone.AsVector = (vector float){-1, -1, -1, -1};
	 perm_v1.AsVector = (vector unsigned char){4, 5, 6, 7, 0, 1, 2, 3, 12, 13, 14, 15, 8, 9, 10, 11};
	...
	for (s = 0; s < (n*inc1)/4; s+=8)
		{
			vec_dst(&cv1[s*4], prefetch_const, 0);

			v1.AsVector = vec_ld(0, &temp1[s*4]);
			v2.AsVector = vec_ld(0, &temp1[(s+1)*4]);
			v3.AsVector = vec_ld(0, &temp1[(s+2)*4]);
			v4.AsVector = vec_ld(0, &temp1[(s+3)*4]);
			v5.AsVector = vec_ld(0, &temp1[(s+4)*4]);
			v6.AsVector = vec_ld(0, &temp1[(s+5)*4]);
			v7.AsVector = vec_ld(0, &temp1[(s+6)*4]);
			v8.AsVector = vec_ld(0, &temp1[(s+7)*4]);	
			
		     
			mul1.AsVector = vec_madd(v1.AsVector, fminusone.AsVector,  kMinusZero);
			mul2.AsVector = vec_madd(v2.AsVector, fminusone.AsVector,  kMinusZero);
			mul3.AsVector = vec_madd(v3.AsVector, fminusone.AsVector,  kMinusZero);
			mul4.AsVector = vec_madd(v4.AsVector, fminusone.AsVector,  kMinusZero);
			mul5.AsVector = vec_madd(v5.AsVector, fminusone.AsVector,  kMinusZero);
			mul6.AsVector = vec_madd(v6.AsVector, fminusone.AsVector,  kMinusZero);
			mul7.AsVector = vec_madd(v7.AsVector, fminusone.AsVector,  kMinusZero);
			mul8.AsVector = vec_madd(v8.AsVector, fminusone.AsVector,  kMinusZero);
			
			
			vec_st(mul1.AsVector, 0, &temp1[s*4]);
			vec_st(mul2.AsVector, 0, &temp1[(s+1)*4]);
			vec_st(mul3.AsVector, 0, &temp1[(s+2)*4]);
			vec_st(mul4.AsVector, 0, &temp1[(s+3)*4]);
			vec_st(mul5.AsVector, 0, &temp1[(s+4)*4]);
			vec_st(mul6.AsVector, 0, &temp1[(s+5)*4]);
			vec_st(mul7.AsVector, 0, &temp1[(s+6)*4]);
			vec_st(mul8.AsVector, 0, &temp1[(s+7)*4]);
	...
	for (s = 0; s < n/4; s+=8)
		{
			...
			l1.AsVector = vec_ld(0, &in_2[s*4]);
			l2.AsVector = vec_ld(0, &in_2[(s+1)*4]);
			l3.AsVector = vec_ld(0, &in_2[(s+2)*4]);
			l4.AsVector = vec_ld(0, &in_2[(s+3)*4]);
			l5.AsVector = vec_ld(0, &in_2[(s+4)*4]);
			l6.AsVector = vec_ld(0, &in_2[(s+5)*4]);
			l7.AsVector = vec_ld(0, &in_2[(s+6)*4]);
			l8.AsVector = vec_ld(0, &in_2[(s+7)*4]);	
			
			
			v1.AsVector = vec_perm(l1.AsVector, l1.AsVector, perm_v1.AsVector); 
		    v2.AsVector = vec_perm(l2.AsVector, l2.AsVector, perm_v1.AsVector);  
		    v3.AsVector = vec_perm(l3.AsVector, l3.AsVector, perm_v1.AsVector);  
		    v4.AsVector = vec_perm(l4.AsVector, l4.AsVector, perm_v1.AsVector);                        
            v5.AsVector = vec_perm(l5.AsVector, l5.AsVector, perm_v1.AsVector); 
		    v6.AsVector = vec_perm(l6.AsVector, l6.AsVector, perm_v1.AsVector); 
		    v7.AsVector = vec_perm(l7.AsVector, l7.AsVector, perm_v1.AsVector); 
		    v8.AsVector = vec_perm(l8.AsVector, l8.AsVector, perm_v1.AsVector); 
			
			
			vec_st(v1.AsVector, 0, &in_2[s*4]);
			vec_st(v2.AsVector, 0, &in_2[(s+1)*4]);
			vec_st(v3.AsVector, 0, &in_2[(s+2)*4]);
			vec_st(v4.AsVector, 0, &in_2[(s+3)*4]);
			vec_st(v5.AsVector, 0, &in_2[(s+4)*4]);
			vec_st(v6.AsVector, 0, &in_2[(s+5)*4]);
			vec_st(v7.AsVector, 0, &in_2[(s+6)*4]);
			vec_st(v8.AsVector, 0, &in_2[(s+7)*4]);
...
int bavrfftf( float *rv1, long inc1, float *cout, long inco, long n, long *p) 
{
	...
	if (rv1 != cout)
	{
		...
		for (s = 0,i=0; s < (n*inc1)/4, i < (2*n*inc1)/4; s+=4,i+=8)
		{
			...
			vec_st(v1.AsVector, 0, &cbuff[s*4]);
			vec_st(v2.AsVector, 0, &cbuff[(s+1)*4]);
			vec_st(v3.AsVector, 0, &cbuff[(s+2)*4]);
			vec_st(v4.AsVector, 0, &cbuff[(s+3)*4]);
			vec_st(v5.AsVector, 0, &cbuff[(s+4)*4]);
			vec_st(v6.AsVector, 0, &cbuff[(s+5)*4]);
			vec_st(v7.AsVector, 0, &cbuff[(s+6)*4]);
			vec_st(v8.AsVector, 0, &cbuff[(s+7)*4]);
...
/***********************************************
Function name: bavrffti

Function description: 
The inverse complex-to-real FFT of the complex input vector is computed and stored in the 
real output vector.

Parameters: 
1. cv1 (inp) - Complex input vector
2. inc1 (inp) - Input vector increment
3. rvo (out) - Real output vector
4. inco (inp) - Output vector increment
...
int bavrffti( float *cv1, long inc1, float *rvo, long inco, long n, long *p) 
{
	int incflag = 0, bufflag = 0, vecflag = 0, cflag = 0;
	float *cv2;
	float *cbuff;
	...
	const vector float kMinusZero = vec_neg_zero();
	vec_Uchar perm_v1,perm_v2;
	vec_float n_scalar, conj;
	
	n_scalar.AsVector = (vector float){1/n, 1/n, 1/n, 1/n};
	conj.AsVector = (vector float){1, -1, 1, -1};
	
	
    perm_v1.AsVector = (vector unsigned char){4, 5, 6, 7, 0, 1, 2, 3, 12, 13, 14, 15, 8, 9, 10, 11};
	perm_v2.AsVector = (vector unsigned char){0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19, 24, 25, 26, 27};
	...
	for (s = 0; s < (n*inc1)/4; s+=8)
		{
			...
			l1.AsVector = vec_ld(0, &cv1[s*4]);
			l2.AsVector = vec_ld(0, &cv1[(s+1)*4]);
			l3.AsVector = vec_ld(0, &cv1[(s+2)*4]);
			l4.AsVector = vec_ld(0, &cv1[(s+3)*4]);
			l5.AsVector = vec_ld(0, &cv1[(s+4)*4]);
			l6.AsVector = vec_ld(0, &cv1[(s+5)*4]);
			l7.AsVector = vec_ld(0, &cv1[(s+6)*4]);
			l8.AsVector = vec_ld(0, &cv1[(s+7)*4]);	
			...
			vec_st(v1.AsVector, 0, &cv1[s*4]);
			vec_st(v2.AsVector, 0, &cv1[(s+1)*4]);
			vec_st(v3.AsVector, 0, &cv1[(s+2)*4]);
			vec_st(v4.AsVector, 0, &cv1[(s+3)*4]);
			vec_st(v5.AsVector, 0, &cv1[(s+4)*4]);
			vec_st(v6.AsVector, 0, &cv1[(s+5)*4]);
			vec_st(v7.AsVector, 0, &cv1[(s+6)*4]);
			vec_st(v8.AsVector, 0, &cv1[(s+7)*4]);
     ...
     /*for(s = 0; s < (n*inc1)/4; s += 8)
	  {
		 ...
		 v1.AsVector = vec_ld(0, &cv1[s*4]);
		 v2.AsVector = vec_ld(0, &cv1[(s+1)*4]);
		 v3.AsVector = vec_ld(0, &cv1[(s+2)*4]);			
		 v4.AsVector = vec_ld(0, &cv1[(s+3)*4]);
		 v5.AsVector = vec_ld(0, &cv1[(s+4)*4]);
		 v6.AsVector = vec_ld(0, &cv1[(s+5)*4]);	
		 v7.AsVector = vec_ld(0, &cv1[(s+6)*4]);
		 v8.AsVector = vec_ld(0, &cv1[(s+7)*4]);	
         
		 v9.AsVector = vec_madd(v1.AsVector, conj.AsVector, kMinusZero);
		 v10.AsVector = vec_madd(v2.AsVector, conj.AsVector, kMinusZero);
		 v11.AsVector = vec_madd(v3.AsVector, conj.AsVector, kMinusZero);
		 v12.AsVector = vec_madd(v4.AsVector, conj.AsVector, kMinusZero);
		 v13.AsVector = vec_madd(v5.AsVector, conj.AsVector, kMinusZero);
		 v14.AsVector = vec_madd(v6.AsVector, conj.AsVector, kMinusZero);
		 v15.AsVector = vec_madd(v7.AsVector, conj.AsVector, kMinusZero);
		 v16.AsVector = vec_madd(v8.AsVector, conj.AsVector, kMinusZero);
         
		 vec_st(v9.AsVector, 0, &cv1[s*4]);
		 vec_st(v10.AsVector, 0, &cv1[(s+1)*4]);
		 vec_st(v11.AsVector, 0, &cv1[(s+2)*4]);
		 vec_st(v12.AsVector, 0, &cv1[(s+3)*4]);
		 vec_st(v13.AsVector, 0, &cv1[(s+4)*4]);
		 vec_st(v14.AsVector, 0, &cv1[(s+5)*4]);
		 vec_st(v15.AsVector, 0, &cv1[(s+6)*4]);
		 vec_st(v16.AsVector, 0, &cv1[(s+7)*4]);
	  }*/
...
/*	for (u = 0; u < n*inc1; u += inc1)
	...
	if ((inc1 != 2))
	{
	  ...
	  cv2 = calloc(n*2, sizeof(float));
		...
		{								
			...
			{
				vec_dst(&cv1[r*4], prefetch_const, 0);

				v1.AsVector = vec_ld(0, &cv1[r*4]);
				v2.AsVector = vec_ld(0, &cv1[(r+1)*4]);
				v3.AsVector = vec_ld(0, &cv1[(r+2)*4]);
				v4.AsVector = vec_ld(0, &cv1[(r+3)*4]);
				v5.AsVector = vec_ld(0, &cv1[(r+4)*4]);
				v6.AsVector = vec_ld(0, &cv1[(r+5)*4]);
				v7.AsVector = vec_ld(0, &cv1[(r+6)*4]);
				v8.AsVector = vec_ld(0, &cv1[(r+7)*4]);	
	...
	if ((incflag == 1) && (bufflag == 0))
	{
		...
		for(r = 0,q=0; r < (n*2)/4,q < (n*inco)/8; r += 8, q +=4)
		{

			vec_dst(&cv2[r*4], prefetch_const, 0);

			l1.AsVector = vec_ld(0, &cv2[r*4]);
			l2.AsVector = vec_ld(0, &cv2[(r+1)*4]);
			l3.AsVector = vec_ld(0, &cv2[(r+2)*4]);
			l4.AsVector = vec_ld(0, &cv2[(r+3)*4]);
			l5.AsVector = vec_ld(0, &cv2[(r+4)*4]);
			l6.AsVector = vec_ld(0, &cv2[(r+5)*4]);
			l7.AsVector = vec_ld(0, &cv2[(r+6)*4]);
			l8.AsVector = vec_ld(0, &cv2[(r+7)*4]);	

			v1.AsVector = vec_perm(l1.AsVector, l2.AsVector, perm_v2.AsVector); 
		    v2.AsVector = vec_perm(l3.AsVector, l4.AsVector, perm_v2.AsVector);  
		    v3.AsVector = vec_perm(l5.AsVector, l6.AsVector, perm_v2.AsVector);  
		    v4.AsVector = vec_perm(l7.AsVector, l8.AsVector, perm_v2.AsVector);                        
		...
		/*	v9.AsVector = vec_madd(v1.AsVector, conj.AsVector, kMinusZero);
			...
			v9.AsVector = vec_madd(v1.AsVector, n_scalar.AsVector, kMinusZero);
		    v10.AsVector = vec_madd(v2.AsVector, n_scalar.AsVector, kMinusZero);
		    v11.AsVector = vec_madd(v3.AsVector, n_scalar.AsVector, kMinusZero);
		    v12.AsVector = vec_madd(v4.AsVector, n_scalar.AsVector, kMinusZero);
			
			vec_st(v9.AsVector, 0, &cv1[q*4]);
			vec_st(v10.AsVector, 0, &cv1[(q+1)*4]);
			vec_st(v11.AsVector, 0, &cv1[(q+2)*4]);
			vec_st(v12.AsVector, 0, &cv1[(q+3)*4]);
	...
	else if ((incflag == 1) && (bufflag == 1))
	{
	  ...
	  for(r = 0,q=0; r < (n*2)/4,q < (n*inco)/8; r += 8, q +=4)
		...
		/*	v9.AsVector = vec_madd(v1.AsVector, conj.AsVector, kMinusZero);
			...
			vec_st(v9.AsVector, 0, &rvo[q*4]);
			vec_st(v10.AsVector, 0, &rvo[(q+1)*4]);
			vec_st(v11.AsVector, 0, &rvo[(q+2)*4]);
			vec_st(v12.AsVector, 0, &rvo[(q+3)*4]);
	...
	else if ((incflag == 0) && (bufflag == 1))
	{
		...
		vec_dst(&cv2[r*4], prefetch_const, 0);
			...
			l1.AsVector = vec_ld(0, &cbuff[r*4]);
			l2.AsVector = vec_ld(0, &cbuff[(r+1)*4]);
			l3.AsVector = vec_ld(0, &cbuff[(r+2)*4]);
			l4.AsVector = vec_ld(0, &cbuff[(r+3)*4]);
			l5.AsVector = vec_ld(0, &cbuff[(r+4)*4]);
			l6.AsVector = vec_ld(0, &cbuff[(r+5)*4]);
			l7.AsVector = vec_ld(0, &cbuff[(r+6)*4]);
			l8.AsVector = vec_ld(0, &cbuff[(r+7)*4]);	
   ...
   /*  for(s = 0; s < (n*inc1)/4; s += 8)
	{
		...
		v1.AsVector = vec_ld(0, &rvo[s*4]);
		v2.AsVector = vec_ld(0, &rvo[(s+1)*4]);
		v3.AsVector = vec_ld(0, &rvo[(s+2)*4]);			
		v4.AsVector = vec_ld(0, &rvo[(s+3)*4]);
		v5.AsVector = vec_ld(0, &rvo[(s+4)*4]);
		v6.AsVector = vec_ld(0, &rvo[(s+5)*4]);	
		v7.AsVector = vec_ld(0, &rvo[(s+6)*4]);
		v8.AsVector = vec_ld(0, &rvo[(s+7)*4]);	
		...
		vec_st(v9.AsVector, 0,  &rvo[s*4]);
		vec_st(v10.AsVector, 0, &rvo[(s+1)*4]);
		vec_st(v11.AsVector, 0, &rvo[(s+2)*4]);
		vec_st(v12.AsVector, 0, &rvo[(s+3)*4]);
		vec_st(v13.AsVector, 0, &rvo[(s+4)*4]);
		vec_st(v14.AsVector, 0, &rvo[(s+5)*4]);
		vec_st(v15.AsVector, 0, &rvo[(s+6)*4]);
		vec_st(v16.AsVector, 0, &rvo[(s+7)*4]);*/
	}

/*		for(u = 0; u < (n*inco)/4; u += 8)
	{
		v1.AsVector = vec_ld(0, &cout[u*4]);
		v2.AsVector = vec_ld(0, &cout[(u+1)*4]);
		v3.AsVector = vec_ld(0, &cout[(u+2)*4]);			
		v4.AsVector = vec_ld(0, &cout[(u+3)*4]);
		v5.AsVector = vec_ld(0, &cout[(u+4)*4]);
		v6.AsVector = vec_ld(0, &cout[(u+5)*4]);	
		v7.AsVector = vec_ld(0, &cout[(u+6)*4]);
		v8.AsVector = vec_ld(0, &cout[(u+7)*4]);	

		v9.AsVector = vec_madd(v1.AsVector, n_scalar.AsVector, kMinusZero);
		v10.AsVector = vec_madd(v2.AsVector, n_scalar.AsVector, kMinusZero);
		v11.AsVector = vec_madd(v3.AsVector, n_scalar.AsVector, kMinusZero);
		v12.AsVector = vec_madd(v4.AsVector, n_scalar.AsVector, kMinusZero);
		v13.AsVector = vec_madd(v5.AsVector, n_scalar.AsVector, kMinusZero);
		v14.AsVector = vec_madd(v6.AsVector, n_scalar.AsVector, kMinusZero);
		v15.AsVector = vec_madd(v7.AsVector, n_scalar.AsVector, kMinusZero);
		v16.AsVector = vec_madd(v8.AsVector, n_scalar.AsVector, kMinusZero);

		vec_st(v9.AsVector, 0, &cout[u*4]);
		vec_st(v10.AsVector, 0, &cout[(u+1)*4]);
		vec_st(v11.AsVector, 0, &cout[(u+2)*4]);
		vec_st(v12.AsVector, 0, &cout[(u+3)*4]);
		vec_st(v13.AsVector, 0, &cout[(u+4)*4]);
		vec_st(v14.AsVector, 0, &cout[(u+5)*4]);
		vec_st(v15.AsVector, 0, &cout[(u+6)*4]);
		vec_st(v16.AsVector, 0, &cout[(u+7)*4]);
	}
*/
	vec_dss(0);		
